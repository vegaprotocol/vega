...
// 	// Generate a Key pair
// 	// _, err = suite.RunMain(ctx, "wallet key generate --output json --no-version-check --root-path %s --passphrase  --name test", path, path)
// 	// require.NoError(t, err)
...
// 		out, err = suite.RunMain(ctx, "wallet sign --output json --no-version-check --root-path %s --passphrase %s/passphrase --name test -m aG9sYQo= -k %s", path, path, pub)
// 		require.NoError(t, err)
// 		sig := out

// 		// Verify
// 		t.Run("Verify", func(t *testing.T) {
// 			out, err = suite.RunMain(ctx, "wallet verify --output json --no-version-check --root-path %s --passphrase %s/passphrase --name test -m aG9sYQo= -k %s -s %s", path, path, pub, sig)
// 			require.NoError(t, err)
