...
func (rp *ReferralPrograms) insertReferralProgram(ctx context.Context, referral *entities.ReferralProgram) error {
	_, err := rp.Connection.Exec(ctx,
		`INSERT INTO referral_programs (id, version, benefit_tiers, end_of_program_timestamp, window_length, vega_time, ended_at)
...
func (rp *ReferralPrograms) GetCurrentReferralProgram(ctx context.Context) (entities.ReferralProgram, error) {
	...
	query := `SELECT id, version, benefit_tiers, end_of_program_timestamp, window_length, vega_time, ended_at FROM current_referral_program`
