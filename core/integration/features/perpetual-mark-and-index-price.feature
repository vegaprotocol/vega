Feature: Simple tests for perpetual market mark price.
  ...
  Scenario: 0053-PERP-033
    When the perpetual oracles from "0xCAFECAFE1":
      ...
      | perp-oracle | ETH   | perp.ETH.value      | TYPE_INTEGER    | perp.funding.cue  | TYPE_TIMESTAMP | 0                     | 0             | 0                 | 0                 | ETH        | 18                  | weight     | 1000        | 0,1,0,0        | 0s,100s,0s,0s              |
    And the markets:
  ...
  Scenario: 0053-PERP-033
    When the perpetual oracles from "0xCAFECAFE1":
      ...
      | perp-oracle | ETH   | perp.ETH.value      | TYPE_INTEGER    | perp.funding.cue  | TYPE_TIMESTAMP | 0                     | 0             | 0                 | 0                 | ETH        | 18                  | weight     | 1000        | 0,1,0,0        | 0s,100s,0s,0s              |
    And the markets:
      | id        | quote name | asset | liquidity monitoring | risk model            | margin calculator         | auction duration | fees         | price monitoring | data source config | linear slippage factor | quadratic slippage factor | decimal places | position decimal places | market type | sla params | price type | source weights |  source staleness tolerance |
      | ETH/DEC19 | ETH        | ETH   | lqm-params           | default-st-risk-model | default-margin-calculator | 1                | default-none | default-none     | perp-oracle        | 0.1                    | 0                         | 5              | 5                       | perp        | SLA        | weight     | 0,0,1,0        |  0s,0,100s,0s               |
    Given the parties submit the following liquidity provision:
      | id  | party  | market id | commitment amount | fee | lp type    |
      | lp1 | lpprov | ETH/DEC19 | 3905000000000000  | 0.3 | submission |
    And the parties place the following pegged iceberg orders:
