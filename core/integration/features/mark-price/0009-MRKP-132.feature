Feature: Test setting of mark price from oracles with one getting stale (AC 0009-MRKP-132)
    Background:
        ...
        And the markets:
            ...
            | ETH/FEB23 | ETH        | USD   | lqm-params           | simple-risk-model | default-margin-calculator | 1                | default-none | default-none     | default-eth-for-future | 0.25                   | 0                         | default-futures | weight     | 1            | 1           | 0           | 0,0,1,1,1,0    | 0s,0s,0s,0s,0s,0s          | oracle1 | oracle2 | oracle3 |
    ...
    Scenario: 001 check mark price using weight average
        ...
        When the network moves ahead "1" blocks
        Then the mark price should be "15946" for the market "ETH/FEB23"

        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value | time offset |
            | price1.USD.value | 16000 | -1s         |
            | price2.USD.value | 17000 | -1s         |

        When the network moves ahead "1" blocks
        Then the mark price should be "16500" for the market "ETH/FEB23"