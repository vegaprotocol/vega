...
const (
	sqlMarketsColumns = `id, tx_hash, vega_time, instrument_id, tradable_instrument, decimal_places,
		...
		parent_market_id, insurance_pool_fraction, liquidity_sla_parameters, liquidation_strategy`
)
...
set
	...
	liquidation_strategy=EXCLUDED.liquidation_strategy;`, sqlMarketsColumns)
	...
	if _, err := m.Connection.Exec(ctx, query, market.ID, market.TxHash, market.VegaTime, market.InstrumentID, market.TradableInstrument, market.DecimalPlaces,
		...
		market.LiquiditySLAParameters, market.LiquidationStrategy); err != nil {
...
select mc.id,  mc.tx_hash,  mc.vega_time,  mc.instrument_id,  mc.tradable_instrument,  mc.decimal_places,
		...
		mc.parent_market_id, mc.insurance_pool_fraction, ml.market_id as successor_market_id, mc.liquidity_sla_parameters, mc.liquidation_strategy
