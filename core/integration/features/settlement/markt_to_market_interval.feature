Feature: Test mark to market settlement with periodicity and different order types, takes the first scenario from mark_to_market_settlement_neg_pdp
  ...
  Scenario: S002 - Test that market settlement cashflows only depend on parties positions and is independent of what order types (testing GTT in this scenario) there are on the book. (0024-OSTA-009)
    Given the following network parameters are set:
      | name                                    | value |
      | network.markPriceUpdateMaximumFrequency | 5s    |
    And the parties deposit on asset's general account the following amount:
