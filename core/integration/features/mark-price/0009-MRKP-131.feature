Feature: Test setting of mark price from 2 oracles - check mark price is updating on schedule (AC 0009-MRKP-131)
    Background:
    ...
    Scenario: 001 check mark price using weight average
        ...
        # 1s into the mark price period
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value | time offset |
            | price1.USD.value | 16000 | -1s         |
        Then the mark price should be "15900" for the market "ETH/FEB23"

        # 2s into the mark price period
        When the network moves ahead "1" blocks
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value | time offset |
            | price2.USD.value | 15000 | -1s         |
        Then the mark price should be "15900" for the market "ETH/FEB23"

        # 3s into the mark price period
        When the network moves ahead "1" blocks
        Then the mark price should be "15900" for the market "ETH/FEB23"

        # 4s into the mark price period
        When the network moves ahead "1" blocks
        Then the mark price should be "15900" for the market "ETH/FEB23"

        # 5s into the mark price period
        When the network moves ahead "1" blocks
        Then the mark price should be "15900" for the market "ETH/FEB23"

        # now mtm has happened
        When the network moves ahead "1" blocks
        Then the mark price should be "15250" for the market "ETH/FEB23"

        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value | time offset |
            | price2.USD.value | 20000 | -1s         |
        When the network moves ahead "3" blocks

        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value | time offset |
            | price1.USD.value | 10000 | -1s         |
        When the network moves ahead "2" blocks
        # 10000*0.25+20000*0.75 = 17500
        Then the mark price should be "17500" for the market "ETH/FEB23"
