...
type Product interface {
	Settle(*num.Uint, uint32, num.Decimal) (*types.FinancialAmount, bool, num.Decimal, error)
	GetAsset() string
}
...
func (e *Engine) Settle(t time.Time, assetDecimals uint32) ([]*types.Transfer, *num.Uint, error) {
	e.log.Debugf("Settling market, closed at %s", t.Format(time.RFC3339))
	positions, round, err := e.settleAll(assetDecimals)
	if err != nil {
		e.log.Error(
...
func (e *Engine) settleAll(assetDecimals uint32) ([]*types.Transfer, *num.Uint, error) {
	e.mu.Lock()
	...
	for _, party := range keys {
		...
		amt, neg, rem, err := e.product.Settle(e.lastMarkPrice, assetDecimals, num.DecimalFromInt64(pos).Div(e.positionFactor))
