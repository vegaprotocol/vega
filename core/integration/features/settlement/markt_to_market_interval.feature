Feature: Test mark to market settlement with periodicity, takes the first scenario from mark_to_market_settlement_neg_pdp
  ...
  Background:
    Given the markets:
      ...
      | ETH/DEC19 | ETH        | ETH   | default-simple-risk-model-3 | default-margin-calculator | 1                | default-none | default-none     | default-eth-for-future | -3                      | 1                      | 0                         | default-futures |
  ...
  Scenario: S001 - Set MTM settlement interval at 5s (the default behaviour), based on first scenario in mark_to_market_settlement_neg_pdp
    ...
    Then the parties should have the following account balances:
      | party  | asset | market id | margin  |  general |
      | party1 | ETH   | ETH/DEC19 | 5280000 |  3720000 |
      | party3 | ETH   | ETH/DEC19 | 2664000 |  7334000 |
      | party2 | ETH   | ETH/DEC19 | 2664000 |  8335000 |
