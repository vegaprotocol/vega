Feature: Test the mark price update candidate gets rejected if it violates the price monitoring engine bounds.
  ...
  Scenario: 0032-PRIM-040
    ...
    #create traded price 16008 which is outside price monitoring bounds
    And the parties place the following orders:
      | party           | market id | side | volume | price | resulting trades | type       | tif     | reference |
    ...
    Then the market data for the market "ETH/FEB23" should be:
      | mark price | trading mode            | horizon | min bound | max bound | target stake | supplied stake | open interest |
      | 16008      | TRADING_MODE_CONTINUOUS | 36000   | 15909     | 16107     | 0            | 0              | 5             |
    ...
    And the parties place the following orders:
      ...
      | sellSideProvider | ETH/FEB23 | sell | 1      | 15920 | 1                | TYPE_LIMIT | TIF_GTC |           |
    When the network moves ahead "1" blocks
    Then the mark price should be "16008" for the market "ETH/FEB23"
    Then the market data for the market "ETH/FEB23" should be:
      | mark price | trading mode            | horizon | min bound | max bound | target stake | supplied stake | open interest |
      | 16008      | TRADING_MODE_CONTINUOUS | 36000   | 15850     | 16408     | 0            | 0              | 6             |
