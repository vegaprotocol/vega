Feature: check when settlement data precision is different/equal to the settlement asset precision, and also check decimal on the market with different market/position/asset decimal
  ...
  @Perpetual
  Scenario: 001 oracle data decimal < asset decimal 0070-MKTD-019
    And the markets:
      ...
      | ETH/DEC19 | ETH        | USD   | default-simple-risk-model-3 | default-margin-calculator | 1                | default-none | default-none     | perp-oracle-3      | 1e6                    | 1e6                       | -1                      | perp        | default-futures |
    And the following network parameters are set:
      | name                           | value |
      | market.auction.minimumDuration | 1     |
      | limits.markets.maxPeggedOrders | 2     |
