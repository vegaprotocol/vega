Feature: Given the program is currently in an automated auction, if a program's expiry timestamp is reached, the program will only be cancelled when the current auction uncrosses at which point no further auctions will take place. (0097-PAPU-023).
    Background:
    ...
    Scenario: PAP order is submitted into a PAP auction and is fully matched - during the auction the pap expires - the auction will end successfully and the pap will be finished then.
        When the protocol automated purchase is defined as:
            ...
            | 12345 | BTC  | ACCOUNT_TYPE_BUY_BACK_FEES | ACCOUNT_TYPE_NETWORK_TREASURY | BTC/ETH   | price_oracle | 10s                              | 1.01                 | auction_schedule        | auction_vol_snap_schedule               | 120s             | 100                  | 200                  | 1727136060       |
        ...
        And the network moves ahead "121" blocks
