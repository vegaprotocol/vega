Feature: Network disposing position
  ...
  Scenario: Network takes over distressed position and disposes position full position once position smaller than full disposal size (0012-POSR-022)(0012-POSR-024)
    ...
    When the parties place the following orders:
      | party | market id | side | volume | price | resulting trades | type       | tif     | reference |
      | lp1   | ETH/MAR23 | buy  | 1000   | 999   | 0                | TYPE_LIMIT | TIF_GTC | best-bid  |
      | lp1   | ETH/MAR23 | sell | 1000   | 1001  | 0                | TYPE_LIMIT | TIF_GTC | best-ask  |
    When the parties place the following orders:
      | party | market id | side | volume | price | resulting trades | type       | tif     |
      | aux1  | ETH/MAR23 | buy  | 1      | 1000  | 0                | TYPE_LIMIT | TIF_GTC |
      | aux2  | ETH/MAR23 | sell | 1      | 1000  | 0                | TYPE_LIMIT | TIF_GTC |
    ...
    Given the parties place the following orders:
      | party       | market id | side | volume | price | resulting trades | type       | tif     |
      | aux1        | ETH/MAR23 | buy  | 10     | 1000  | 0                | TYPE_LIMIT | TIF_GTC |
      | atRiskParty | ETH/MAR23 | sell | 10     | 1000  | 1                | TYPE_LIMIT | TIF_GTC |
    Then the network moves ahead "1" blocks
    ...
    And the parties should have the following margin levels:
      | party       | market id | maintenance | search | initial | release |
      | atRiskParty | ETH/MAR23 | 156         | 171    | 187     | 218     |
    And the parties should have the following account balances:
      | party       | asset    | market id | margin | general |
      | atRiskParty | USD.0.10 | ETH/MAR23 | 175    | 5       |
    ...
    And the parties place the following orders:
      | party | market id | side | volume | price | resulting trades | type       | tif     |
      | aux1  | ETH/MAR23 | buy  | 1      | 1010  | 0                | TYPE_LIMIT | TIF_GTC |
      | aux2  | ETH/MAR23 | sell | 1      | 1010  | 1                | TYPE_LIMIT | TIF_GTC |
    Then the network moves ahead "1" blocks
    Then the market data for the market "ETH/MAR23" should be:
      | mark price | trading mode            |
      | 1010       | TRADING_MODE_CONTINUOUS |
