...
Feature: Test margin for lp near price monitoring boundaries
  ...
  Scenario: first scenario for volume at near price monitoring bounds and simple-risk-model
    ...
    And the markets:
      ...
      | ETH/DEC21 | ETH        | ETH   | simple-risk-model-1 | default-margin-calculator | 1                | fees-config-1 | price-monitoring-1 | default-eth-for-future | 1e6                    | 1e6                       | 10m         |
  ...
  Scenario: second scenario for volume at near price monitoring bounds with log-normal
    ...
    And the markets:
      | id         | quote name | asset | risk model              | margin calculator         | auction duration | fees          | price monitoring   | data source config     | linear slippage factor | quadratic slippage factor | lp timestep |
      | ETH2/MAR22 | ETH2       | ETH2  | log-normal-risk-model-1 | default-margin-calculator | 1                | fees-config-1 | price-monitoring-2 | default-eth-for-future | 1e6                    | 1e6                       | 10m         |
    And the parties deposit on asset's general account the mollowing amount:
      | party  | asset | amount    |
      | lp1    | ETH2  | 100000000 |
