...
func TestRewards(t *testing.T) {
	...
	t.Run("GetByCursor", func(t *testing.T) {
		...
		actual, _, err := rs.GetByCursor(ctx, []string{party1ID, party2ID}, &asset2ID, nil, nil, pagination, nil, nil, nil)
		require.NoError(t, err)
		assert.Equal(t, expected, actual)
	})
}
...
func testRewardsCursorPaginationNoPagination(t *testing.T) {
	...
	got, pageInfo, err := rs.GetByCursor(ctx, []string{partyID}, &assetID, nil, nil, pagination, nil, nil, nil)
...
func TestRewardFilterByTeamIDAndGameID(t *testing.T) {
	...
	t.Run("Should return rewards for all teams the party has been a member of if no team ID is provided", func(t *testing.T) {
		...
		got, _, err := stores.rewards.GetByCursor(ctx, []string{partyID.String()}, nil, nil, nil, page, nil, nil, nil)
		require.NoError(t, err)
	...
	t.Run("Should return rewards for the specified team if a team ID is provided", func(t *testing.T) {
		...
		page := entities.DefaultCursorPagination(true)
		got, _, err := stores.rewards.GetByCursor(ctx, []string{party.ID.String()}, nil, nil, nil, page, ptr.From(teamID), nil, nil)
		require.NoError(t, err)
	...
	t.Run("Should return rewards for the specified game if a game ID is provided", func(t *testing.T) {
		...
		gameID := gameIDs[i]
		page := entities.DefaultCursorPagination(true)
		got, _, err := stores.rewards.GetByCursor(ctx, nil, nil, nil, nil, page, nil, ptr.From(gameID), nil)
		require.NoError(t, err)
