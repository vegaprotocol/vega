...
func TestMain(t *testing.M) {
	...
	exitCode := databasetest.TestMain(t, func(config sqlstore.Config, source *sqlstore.ConnectionSource,
		...
		networkHistoryService, err = networkhistory.NewWithStore(outerCtx, log, chainID, cfg, sqlConfig.ConnectionConfig, snapshotService,
		...
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[1000].HistorySegmentID, "QmdBw78Mh5FjPguhxYbfSQA8LRCgZFftFz6ZHztphvWEbY", snapshots)
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[2000].HistorySegmentID, "QmU8uCmXr5M3mtJetpu3T2jaSun8zUWz5dV2YgkraRThRF", snapshots)
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[2500].HistorySegmentID, "QmSmUtKgcs4gUBB7SLnFE7GMJyuKqm815ru7p8q4kHXsVc", snapshots)
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[3000].HistorySegmentID, "QmUiHCP4kacNqZqEfvX9U2Aq4gBRPXw3nKswmMg2PXgyE1", snapshots)
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[4000].HistorySegmentID, "QmRyDrmWY2yTXP4gB48uYhKFf5rmsBo7m4me4Fu5vx2YPw", snapshots)
		panicIfHistorySegmentIdsNotEqual(goldenSourceHistorySegment[5000].HistorySegmentID, "QmUtW7qQdpi4jma7saMYAEaLFw2SFWTyQehgh4Q31z6sHx", snapshots)
...
func TestRestoringNodeWithDataOlderAndNewerThanItContainsLoadsTheNewerData(t *testing.T) {
	...
	result, err := networkhistoryService.LoadNetworkHistoryIntoDatanode(ctx)
	require.Nil(t, err)
...
func TestRestoringNodeWithHistoryOnlyFromBeforeTheNodesOldestBlockFails(t *testing.T) {
	...
	_, err = networkhistoryService.LoadNetworkHistoryIntoDatanode(ctx)
...
func TestRestoreFromPartialHistoryAndProcessEvents(t *testing.T) {
	...
	outputSnapshotService := setupSnapshotService(sqlConfig, outNetworkHistoryHome, t.TempDir())
...
func setupNetworkHistoryService(ctx context.Context, log *logging.Logger, inputSnapshotService *snapshot.Service, store *store.Store,
	...
	datanodeConfig := config2.NewDefaultConfig()
	networkHistoryService, err := networkhistory.NewWithStore(ctx, log, chainID, cfg, sqlConfig.ConnectionConfig,
