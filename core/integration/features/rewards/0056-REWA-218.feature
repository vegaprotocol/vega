Feature: Team Rewards
    ...
    @TeamStep
    Scenario: Given a recurring transfer with a reward window `>1` and specifying a non-zero `target_notional_volume`. When the volume is zero in the current epoch, if the cumulative volume across the window is less than the target, the rewards distributed are scaled accordingly. (<a name="0056-REWA-218" href="#0056-REWA-218">0056-REWA-218</a>).
        ...
        # notional is uneven across the window (no notional in last epoch), but total notional traded over the 2 epochs is 40% of the target notional
