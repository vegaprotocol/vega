Feature: Amending order trades in full, plenty of balance to cover the order amendment + trades + fees
  Background:
  ...
  Scenario:
    ...
    # amend the order to increase the required margin, and the order trades in full
    When the parties amend the following orders:
      | party   | reference   | price | size delta | tif     | error |
      | trader3 | t3-to-amend | 15802 | -90        | TIF_GTC |       |
    Then the orders should have the following status:
      | party   | reference   | status        |
      | trader3 | t3-to-amend | STATUS_FILLED |
    And the parties should have the following margin levels:
      | party   | market id | maintenance | search | initial | release | margin mode     | margin factor | order |
      | trader3 | ETH/FEB23 | 5243        | 0      | 6291    | 0       | isolated margin | 0.3           | 0     |
    And the parties should have the following account balances:
      | party   | asset | market id | margin | general     |
      | trader3 | USD   | ETH/FEB23 | 14694  | 99999985147 |
