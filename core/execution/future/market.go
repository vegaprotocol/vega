...
func (m *Market) submitValidatedOrder(ctx context.Context, order *types.Order) (*types.OrderConfirmation, []*types.Order, error) {
	...
	if marginMode == types.MarginModeIsolatedMargin {
		...
		posWithTrades := pos.UpdateInPlaceOnTrades(m.log, order.Side, trades)
		// 	First, the core will check whether the order will trade, either fully or in part, immediately upon entry. If so:
		...
		if len(trades) > 0 {
			if err := m.checkIsolatedMarginOnImmediateTrade(ctx, posWithTrades, order, trades); err != nil {
				if m.log.GetLevel() <= logging.DebugLevel {
		...
		if err := m.checkIsolatedMarginForOrder(ctx, posWithTrades, order); err != nil {
			if m.log.GetLevel() <= logging.DebugLevel {
				m.log.Debug("Unable to check/add margin for party",
					logging.Order(*order), logging.Error(err))
			}
	...
	for idx, trade := range conf.Trades {
		...
		for _, mp := range m.position.Update(ctx, trade, conf.PassiveOrdersAffected[idx], conf.Order) {
			...
			if m.getMarginMode(conf.PassiveOrdersAffected[idx].Party) == types.MarginModeIsolatedMargin {
				...
				r, err := m.updateIsolatedMarginsOnPositionChange(ctx, pos, conf.PassiveOrdersAffected[idx], trade)
				if err != nil {
					m.log.Error("failed to update isolated margin on position change", logging.Error(err))
				} else {
					m.transferMargins(ctx, []events.Risk{r}, nil)
				}
				// recheck isolated order margin and stop active orders if there are insufficient funds, if they are insufficient, we cancel all
				// outstanding orders
				_, err = m.recalcIsolatedMargins(ctx, pos, m.matching.GetOrdersPerParty(conf.PassiveOrdersAffected[idx].Party))
				if err != nil {
					m.handleIsolatedMarginInsufficientOrderMargin(ctx, conf.PassiveOrdersAffected[idx].Party, err)
				}
			}
			...
			if m.as.InAuction() {
				...
				aggressorOrder, _ := m.matching.GetOrderByID(trade.BuyOrder)
				if trade.Aggressor == types.SideSell {
					aggressor = trade.Seller
					aggressorOrder, _ = m.matching.GetOrderByID(trade.SellOrder)
				}
				if m.getMarginMode(aggressor) == types.MarginModeIsolatedMargin {
					pos, _ := m.position.GetPositionByPartyID(aggressor)
					r, err := m.updateIsolatedMarginsOnPositionChange(ctx, pos, aggressorOrder, trade)
					if err != nil {
						m.log.Error("failed to update isolated margin on position change", logging.Error(err))
					}
					m.transferMargins(ctx, []events.Risk{r}, nil)
					// recheck isolated order margin and stop active orders if there are insufficient funds, if they are insufficient, we cancel all
					// outstanding orders
					_, err = m.recalcIsolatedMargins(ctx, pos, m.matching.GetOrdersPerParty(aggressor))
					if err != nil {
						m.handleIsolatedMarginInsufficientOrderMargin(ctx, aggressor, err)
					}
				}
			}
		}
...
func (m *Market) checkIsolatedMarginOnImmediateTrade(ctx context.Context, pos *positions.MarketPosition, order *types.Order, trades []*types.Trade) error {
	risk, err := m.calcIsolatedMarginsOnNewOrderTraded(ctx, pos, order, trades)
	if err != nil {
		return err
	}
...
func (m *Market) checkIsolatedMarginForOrder(ctx context.Context, pos *positions.MarketPosition, order *types.Order) error {
	risk, err := m.calcIsolatedMarginsOrder(ctx, pos, order, m.matching.GetOrdersPerParty(pos.Party()))
	if err != nil {
		return err
	}
	return m.transferMargins(ctx, risk, nil)
...
// checkIsolatedMarginForAmendOrder is called when a previously parked order becomes unparked as it's being amended.
func (m *Market) checkIsolatedMarginForAmendOrder(ctx context.Context, existingOrder *types.Order, amendedOrder *types.Order) error {
	origPos, ok := m.position.GetPositionByPartyID(existingOrder.Party)
	if !ok {
		m.log.Panic("could not get position for party", logging.PartyID(existingOrder.Party))
	}
	...
	// we are just checking here if we can pass the margin calls.
	risk, err := m.calcIsolatedMarginsOrder(ctx, pos, amendedOrder, m.matching.GetOrdersPerParty(pos.Party()))
	if err != nil {
		return err
	}
	// margins calculated, set about tranferring funds. At this point, if closed is not empty, those parties are distressed
...
func (m *Market) cancelOrder(ctx context.Context, partyID, orderID string) (*types.OrderCancellationConfirmation, error) {
	...
	if m.getMarginMode(partyID) == types.MarginModeIsolatedMargin {
		...
		rr, err := m.recalcIsolatedMargins(ctx, pos, m.matching.GetOrdersPerParty(partyID))
		if err != nil {
			m.log.Panic("failed to recalculate isolated margin after order cancellation", logging.String("party", partyID), logging.Order(order))
		}
		m.transferMargins(ctx, rr, nil)
	}
...
func (m *Market) parkOrder(ctx context.Context, orderID string) *types.Order {
	...
	if m.getMarginMode(order.Party) == types.MarginModeIsolatedMargin {
		rr, err := m.recalcIsolatedMargins(ctx, pos, m.matching.GetOrdersPerParty(order.Party))
		if err != nil {
...
func (m *Market) handleIsolatedMarginInsufficientOrderMargin(ctx context.Context, party string, err error) {
	orders := m.matching.GetOrdersPerParty(party)
	for _, o := range orders {
		m.unregisterAndReject(ctx, o, err)
		if o.PeggedOrder != nil {
			m.removePeggedOrder(o)
		}
	...
	r, err := m.recalcIsolatedMargins(ctx, pos, []*types.Order{})
	if err != nil {
		m.log.Panic("failed to recalc isolated margin", logging.String("party", party))
	}
	m.transferMargins(ctx, r, nil)
}
...
func (m *Market) AmendOrderWithIDGenerator(
	...
	if err != nil {
		if m.getMarginMode(party) == types.MarginModeIsolatedMargin && err == common.ErrMarginCheckFailed {
			m.handleIsolatedMarginInsufficientOrderMargin(ctx, party, err)
		}
		return nil, err
	}
...
func (m *Market) amendOrder(
	...
	if existingOrder.PeggedOrder != nil {
		...
		if amendedOrder.Status == types.OrderStatusParked {
			...
			if m.getMarginMode(existingOrder.Party) == types.MarginModeCrossMargin {
				if err := m.checkMarginForAmendOrder(ctx, existingOrder, amendedOrder); err != nil {
					return nil, nil, err
				}
			} else {
				if err := m.checkIsolatedMarginForAmendOrder(ctx, existingOrder, amendedOrder); err != nil {
					return nil, nil, err
				}
			}
...
func (m *Market) orderCancelReplace(
	...
	if m.as.InAuction() {
		...
		if m.getMarginMode(newOrder.Party) == types.MarginModeIsolatedMargin {
			...
			if err := m.checkIsolatedMarginForOrder(ctx, pos, newOrder); err != nil {
				if m.log.GetLevel() <= logging.DebugLevel {
					m.log.Debug("Unable to check/add margin for party",
	...
	if marginMode == types.MarginModeIsolatedMargin {
		pos, _ := m.position.GetPositionByPartyID(newOrder.Party)
		posWithTrades := pos.UpdateInPlaceOnTrades(m.log, newOrder.Side, trades)
		if len(trades) > 0 {
			// NB: this is the position with the trades included and the order sizes updated to remaining!!!
			if err := m.checkIsolatedMarginOnImmediateTrade(ctx, posWithTrades, newOrder, trades); err != nil {
				if m.log.GetLevel() <= logging.DebugLevel {
					m.log.Debug("Unable to check/add immediate trade margin for party",
						logging.Order(*newOrder), logging.Error(err))
				}
		...
		if err := m.checkIsolatedMarginForOrder(ctx, posWithTrades, newOrder); err != nil {
			if m.log.GetLevel() <= logging.DebugLevel {
				m.log.Debug("Unable to check/add margin for party",
					logging.Order(*newOrder), logging.Error(err))
			}
...
func (m *Market) switchMarginMode(ctx context.Context, party string, marginMode types.MarginMode, marginFactor num.Decimal) error {
	...
	marketObservablae := m.getMarketObservable(nil)
	if marketObservablae == nil {
		return fmt.Errorf("no market observable price")
	}
	increment := m.tradableInstrument.Instrument.Product.GetMarginIncrease(m.timeService.GetTimeNow().UnixNano())
	var auctionPrice *num.Uint
	if m.as.InAuction() {
		auctionPrice = marketObservablae
		markPrice := m.getCurrentMarkPrice()
		if markPrice != nil && marketObservablae.LT(markPrice) {
	...
	if marginMode == types.MarginModeIsolatedMargin {
		...
		err = m.transferMargins(ctx, risk, nil)
		if err != nil {
			return err
		}
		...
		risk := m.risk.SwitchFromIsolatedMargin(margins, marketObservablae, increment)
		if err != nil {
			return err
		}
