Feature: Spot Markets
  ...
  Scenario: If the fee rates change for any reason within an auction, the amount required to cover fees must be recalculated at uncrossing time, 
            and if the user does not have enough to cover the fees the order will be cancelled (0080-SPOT-021).
    ...
    # Let's change the infrastructure fees so that we no longer have enough funds to cover our order
    Given the following network parameters are set:
      | name                                 | value |
    ...
    # The market will attempt to move the required amount of fees (250) for the orders to the holding account ready for the uncrossing,
  ...
  Scenario: If the fee rates change for any reason within an auction, the amount required to cover fees must be recalculated at uncrossing time, 
    ...
    # Let's change the infrastructure fees so that we have more holding value than we need
    Given the following network parameters are set:
      | name                                 | value |
    ...
    # The market will check the holding amount for each order is enough before we uncross. If we have too much in the 
    # holding account we do not reduce it yet, that will happen once the orders are uncrossed.

    And the orders should have the following status:
      | party   | reference    | status        |
      | party3  | bla-bla      | STATUS_ACTIVE |

    # We move out of auction and all fees for orders for party3 will be returned from the holding account to the general account
    Then "party3" should have holding account balance of "1000" for asset "ETH"
    Then "party3" should have general account balance of "200" for asset "ETH"
