...
func TestPriceLevelAddsOrdersWithDifferentTimestampCorrectly(t *testing.T) {
	...
	*/
	...
	assert.Equal(t, len(ordersSitingAtPriceLevel), fetchedTestPriceLevel.orders.Len())
	...
	for element != nil {
		...
		expectOrder(t, orderEntry.order, ordersSitingAtPriceLevel[index].order)
	...
	assert.Equal(t, 0, fetchedTestPriceLevel.orders.Len())
}
...
func TestPriceLevelAddsOrdersWithSameTimestampCorrectly(t *testing.T) {
	...
	*/
	...
	assert.Equal(t, len(ordersSitingAtPriceLevel), testPriceLevel.orders.Len())
