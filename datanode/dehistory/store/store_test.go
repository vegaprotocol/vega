...
func TestRemoveWithNoEligibleSegments(t *testing.T) {
	...
	dehistoryHome := t.TempDir()

	s, snapshotsDir := createStore(t, 20000, chainID, dehistoryHome)
...
func TestPartialRemoveOfOldSegments(t *testing.T) {
	...
	s, snapshotsDir := createStore(t, 5000, chainID, dehistoryHome)
...
func TestRemoveAllOldSegments(t *testing.T) {
	...
	s, snapshotsDir := createStore(t, 0, chainID, dehistoryHome)
...
func createStore(t *testing.T, historyRetentionBlockSpan int64, chainID string, dehistoryHome string) (*store.Store, string) {
	t.Helper()
	...
	s, err := store.New(context.Background(), log, chainID, cfg, dehistoryHome, false)
	require.NoError(t, err)
