...
func TestCancelTransferInstruction(t *testing.T) {
	...
	transferInstructionID := "TRANSFERID"
	...
	// now we try to cancel an non-existing transfer instruction
	assert.EqualError(t,
		e.CancelTransferFunds(ctx, &types.CancelTransferInstructionFunds{TransferInstructionID: "NOPE"}),
		banking.ErrRecurringTransferInstructionDoesNotExists.Error(),
	)

	// now we try to cancel the right transfer, but with the wrong party
	assert.EqualError(t,
		e.CancelTransferFunds(ctx, &types.CancelTransferInstructionFunds{
			TransferID: transferInstructionID,
			Party:      "NOPE",
		}),
	...
	e.col.EXPECT().TransferFunds(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(1).DoAndReturn(
		func(ctx context.Context,
			...
			t.Run("ensure transfers are correct", func(t *testing.T) {
				...
				assert.Equal(t, transferInstructions[0].Amount.Amount, num.NewUint(100))
			...
			t.Run("ensure fee transfers are correct", func(t *testing.T) {
				...
				assert.Equal(t, feeTransferInstructions[0].Amount.Amount, num.NewUint(50))
				assert.Equal(t, feeTransferInstructions[0].Amount.Asset, "eth")

				// then the fees account types
				assert.Len(t, feeTransferInstructionsAccountTypes, 1)
				assert.Equal(t, accountTypes[0], types.AccountTypeGeneral)
			})
	...
	e.broker.EXPECT().Send(gomock.Any()).DoAndReturn(func(evt events.Event) {
		t.Run("ensure transfer  is done", func(t *testing.T) {
			e, ok := evt.(*events.TransferInstructionFunds)
			assert.True(t, ok, "unexpected event from the bus")
			assert.Equal(t, e.Proto().Status, types.TransferInstructionStatusCancelled)
		})
	})
