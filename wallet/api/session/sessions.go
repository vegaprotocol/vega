...
func (s *Sessions) ConnectWalletForLongLivingConnection(
	token string, w wallet.Wallet, now time.Time, expiry *time.Time) error {
...
func (c *ConnectedWallet) Expired(now time.Time) error {
	if c.Expiry != nil && c.Expiry.Before(now) {
