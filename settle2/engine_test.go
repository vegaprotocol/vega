...
import (
	...
	"code.vegaprotocol.io/vega/settle2"
	"code.vegaprotocol.io/vega/settle2/mocks"
...
func testMarkToMarketOrdered(t *testing.T) {
	...
	t.Logf("%#v\n", longTransfer)
	// now, let's update the state again as if the settle2 hasn't happened
	engine.Update(init)
	engine.Update(short)
...
func (te *testEngine) getMockMarketPositions(data []posValue) ([]settle2.MarketPosition, []events.MarketPosition) {
	raw, evts := make([]settle2.MarketPosition, 0, len(data)), make([]events.MarketPosition, 0, len(data))
...
func getTestEngine(t *testing.T) *testEngine {
	...
	conf := settle2.NewDefaultConfig()
	prod := mocks.NewMockProduct(ctrl)
	market := "BTC/DEC19"
	prod.EXPECT().GetAsset().AnyTimes().Do(func() string { return "BTC" })
	return &testEngine{
		Engine:    settle2.New(logging.NewTestLogger(), conf, prod, market),
		ctrl:      ctrl,
		prod:      prod,
		positions: nil,
		market:    market,
