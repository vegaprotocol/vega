...
- Now open ideally two terminal windows. In the first one launch `tendermint` as you normally would. To hard-reset the chain and start from scratch use
    ...
    ```bash
    dlv exec /path/to/data-node/cmd/data-node/data-node-dbg --headless --listen=:2345 --log --api-version=2 -- node
    ```
    ...
    2019-10-13T20:37:41+01:00 info layer=debugger launching process with args: [/path/to/data-node/cmd/data-node/data-node-dbg node]
    debugserver-@(#)PROGRAM:LLDB  PROJECT:lldb-1100.0.28..1
     for x86_64.
    Got a connection, launched process /path/to/data-node/cmd/data-node/data-node (pid = 35671).
    ```
