...
func (m *Market) uncrossOnLeaveAuction(ctx context.Context) ([]*types.OrderConfirmation, []*types.Order) {
	...
	for _, uncrossedOrder := range uncrossedOrders {
		...
		// if the uncrossed order was generated by an AMM then register its position as if it was submitted
		order := uncrossedOrder.Order
		if order.GeneratedOffbook {
			cpy := order.Clone()
			cpy.Remaining = cpy.Size // remaining will be 0 since it has traded, so we copy it back to its full size to register
			m.position.RegisterOrder(ctx, cpy)
		}
...
func (m *Market) checkPriceAndGetTrades(ctx context.Context, order *types.Order) ([]*types.Trade, error) {
	...
	for _, t := range trades {
		if m.pMonitor.CheckPrice(ctx, m.as, t.Price, persistent, false) {
...
func (m *Market) handleConfirmationPassiveOrders(
	...
	if conf.PassiveOrdersAffected != nil {
		...
		for _, order := range conf.PassiveOrdersAffected {
			// if the order was generated by an offbook source such as an AMM we need to register the order here
			// since it was never registered as an incoming order.
			if order.GeneratedOffbook {
				cpy := order.Clone()
				cpy.Remaining = cpy.Size // remaining will be 0 since it has traded, so we copy it back to its full size to register
				m.position.RegisterOrder(ctx, cpy)
...
func (m *Market) dataSourcePropagation(ctx context.Context, dataSourceID string, assetPrice *num.Uint) {
	...
	for i, p := range pools {
		...
		if err := VerifyAMMBounds(params, m.capMax, m.priceFactor); err != nil {
			m.log.Error("unable to update AMM base price from data source", logging.Error(err))
			continue
		}
...
func (m *Market) SubmitAMM(ctx context.Context, submit *types.SubmitAMM, deterministicID string) error {
	...
	// rebase successful so confirm the pool with the engine
	m.amm.Confirm(ctx, pool)
