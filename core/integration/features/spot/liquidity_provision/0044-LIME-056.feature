Feature: Spot market SLA
  ...
  Scenario: 0044-LIME-056, The LP can cancel their commitment at any time (though this may involve incurring a penalty)
    ...
    When party "lpprov1" cancels their liquidity provision for market "BTC/ETH"

    Then the network moves ahead "7" blocks

    Then the liquidity provisions should have the following states:
      | id  | party   | market  | commitment amount | status           |
      | lp1 | lpprov1 | BTC/ETH | 0                 | STATUS_CANCELLED |

    Then the market data for the market "BTC/ETH" should be:
      | mark price | trading mode            | auction trigger             | target stake | supplied stake |
      | 15         | TRADING_MODE_CONTINUOUS | AUCTION_TRIGGER_UNSPECIFIED | 3200         | 2000           |
    # since earlyExitPenalty=0.25, so bond slashed is: 1200*0.25=300
