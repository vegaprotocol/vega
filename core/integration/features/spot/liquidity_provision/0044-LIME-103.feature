Feature: Check LP slashing after intra-epoch change to time on book requirements
  Background:
  ...
  Scenario: If a liquidity provider has fraction_of_time_on_book >= market.liquidity.commitmentMinTimeFraction, no penalty will be taken from their bond account (0044-LIME-103)
    ...
    # Move forward 1 block to end the epoch and make sure we still are punished as SLA should not have updated yet
    And the parties should have the following account balances:
      | party  | asset | market id | general   | bond |
      | party1 | ETH   | BTC/ETH   | 100009000 | 890  |    
