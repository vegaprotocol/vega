...
func TestEvents_CloseOutTraderWithLPOrder(t *testing.T) {
	...
	mdb := subscribers.NewMarketDepthBuilder(ctx, nil, true)
	tm := startMarketInAuction(t, ctx, &now)
	addAccountWithAmount(tm, "trader-Z", 2500)
	// place some orders on the book for when we leave auction
	addAccountWithAmount(tm, "trader-X", 100000000)
	addAccountWithAmount(tm, "trader-Y", 100000000)

	orders := []*types.Order{
		getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GFA, "auctionOrder1", types.Side_SIDE_SELL, "trader-X", 1, 10),
		getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GFA, "auctionOrder2", types.Side_SIDE_BUY, "trader-Y", 1, 10),
		getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "auctionOrder3", types.Side_SIDE_SELL, "trader-X", 1, 11),
		getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "auctionOrder4", types.Side_SIDE_BUY, "trader-Y", 1, 9),
	}
	for _, o := range orders {
		_, err := tm.market.SubmitOrder(ctx, o)
		assert.NoError(t, err)
	...
	now = now.Add(time.Second * 20)
	tm.market.OnChainTimeUpdate(ctx, now)
	assert.Equal(t, types.Market_TRADING_MODE_CONTINUOUS, tm.market.GetMarketData().MarketTradingMode)

	o1 := getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GFN, "Order01", types.Side_SIDE_SELL, "trader-Z", 10, 10)
	o1conf, err := tm.market.SubmitOrder(ctx, o1)
	require.NotNil(t, o1conf)
	require.NoError(t, err)

	o4 := getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "Order04", types.Side_SIDE_BUY, "trader-B", 10, 10)
	o4conf, err := tm.market.SubmitOrder(ctx, o4)
	require.NotNil(t, o4conf)
	require.NoError(t, err)

	o5 := getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "Order05", types.Side_SIDE_SELL, "trader-B", 100, 110)
	o5conf, err := tm.market.SubmitOrder(ctx, o5)
	require.NotNil(t, o5conf)
	require.NoError(t, err)

	o2 := getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "Order02", types.Side_SIDE_BUY, "trader-B", 100, 99)
	o2conf, err := tm.market.SubmitOrder(ctx, o2)
	require.NotNil(t, o2conf)
	require.NoError(t, err)
	...
	o6 := getMarketOrder(tm, now, types.Order_TYPE_LIMIT, types.Order_TIME_IN_FORCE_GTC, "Order06", types.Side_SIDE_SELL, "trader-C", 1, 100)
	o6conf, err := tm.market.SubmitOrder(ctx, o6)
	assert.Equal(t, types.Market_TRADING_MODE_CONTINUOUS, tm.market.GetMarketData().MarketTradingMode)
	...
	// Check we have the right amount of events
	assert.Equal(t, uint64(19), tm.orderEventCount)
	assert.Equal(t, int64(3), tm.market.GetOrdersOnBookCount())
	processEvents(t, tm, mdb)
	assert.Equal(t, int64(3), mdb.GetOrderCount(tm.market.GetID()))
	assert.Equal(t, uint64(0), mdb.GetOrderCountAtPrice(tm.market.GetID(), types.Side_SIDE_SELL, 100))
	assert.Equal(t, uint64(0), mdb.GetVolumeAtPrice(tm.market.GetID(), types.Side_SIDE_SELL, 100))
	assert.Equal(t, 0, tm.market.GetPeggedOrderCount())
	assert.Equal(t, 0, tm.market.GetParkedOrderCount())
}
