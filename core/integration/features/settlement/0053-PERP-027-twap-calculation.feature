Feature: Test internal and external twap calculation
    ...
    Scenario: 0053-PERP-027 Internal and External TWAP calculation
        ...
        When the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value               | time offset |
            | perp.ETH.cue     | 9000000000000000000 | -1s         |
            | perp.funding.cue | 1602806580          | 0s          |
        ...
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value                | time offset |
            | perp.ETH.cue     | 10000000000000000000 | -1s         |
        ...
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value                | time offset |
            | perp.ETH.cue     | 12000000000000000000 | -1s         |
            | perp.funding.cue | 1602806820           | 0s          |

        # 6 min in to the funding period emit spot price
        Given time is updated to "2020-10-16T00:08:00Z"
        And the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value                | time offset |
            | perp.ETH.cue     | 11000000000000000000 | -1s         |
            | perp.funding.cue | 1602806880           | 0s          |
        ...
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value               | time offset |
            | perp.ETH.cue     | 8000000000000000000 | -1s         |
            | perp.funding.cue | 1602806940          | 0s          |
        ...
        Then the oracles broadcast data with block time signed with "0xCAFECAFE1":
            | name             | value                | time offset |
            | perp.ETH.cue     | 14000000000000000000 | -1s         |
            | perp.funding.cue | 1602807060           | 0s          |

        Given time is updated to "2020-10-16T00:13:00Z"
        When the network moves ahead "10" blocks
        Then the mark price should be "7" for the market "ETH/DEC19"

# TODO: Check the internal TWAP = 9.3
# TODO: Check the external TWAP = 10.3
