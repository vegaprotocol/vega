Feature: Test change of SLA market parameter
  ...
  Scenario: 001: lp1 and lp2 on the market BTC/ETH, 0044-LIME-091, 0044-LIME-115
    ...
    Then the spot markets are updated:
      | id      | risk model            | price monitoring | data source config     | sla params |
      | BTC/ETH | log-normal-risk-model | price-monitoring | default-eth-for-future | SLA-22-1   |

    Then the network moves ahead "2" epochs

    #0044-LIME-115:price range in SLA parameter is getting narrower, changes from 0.5 to 0.1
    Then the spot markets are updated:
      | id      | risk model            | price monitoring | data source config     | sla params |
      | BTC/ETH | log-normal-risk-model | price-monitoring | default-eth-for-future | SLA-22-2   |
