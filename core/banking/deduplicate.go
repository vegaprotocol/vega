...
func (e *Engine) deduplicateAssetAction(ctx context.Context, action *assetAction) error {
	ref := action.getRef()
	...
	if e.seenAssetActions.Contains(refKey) {
		if err := e.dedupAction(ctx, action); err != nil {
			e.log.Error("unable to deduplicate action", logging.String("action", action.String()), logging.Error(err))
		}
