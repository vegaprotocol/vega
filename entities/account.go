...
// In vega core, account IDs are generated by concatenating (party id, market id, asset id, account type)
// with '!' indicating no market, and "*" indicating the system owner party. (see collateral/engine.go)
// It's a bit unfortunate that this internal detail leaks out of core, but it's required for now as it's
// the only way we can extract all the required account information out of an TransferResponse message.
