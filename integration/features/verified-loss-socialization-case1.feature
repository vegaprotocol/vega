...
# trader 1 place an order + we check margins
    Then traders place following orders:
      ...
      | trader1 | ETH/DEC19 | sell |    100 |   100 |                0 | LIMIT | TIF_GTC |
# then trader2 place an order, and we calculate the margins again
    Then traders place following orders:
      | trader  | id        | type | volume | price | resulting trades | type  | tif |
      | trader2 | ETH/DEC19 | buy  |    100 |   100 |                1 | LIMIT | TIF_GTC |
# then we change the volume in the book
    ...
    Then traders place following orders with references:
      | trader           | id        | type | volume | price | resulting trades | type  | tif | reference       |
      | sellSideProvider | ETH/DEC19 | sell |   1000 |   200 |                0 | LIMIT | TIF_GTC | sell-provider-2 |
      | buySideProvider  | ETH/DEC19 | buy  |   1000 |    80 |                0 | LIMIT | TIF_GTC | buy-provider-2  |
    Then traders place following orders:
      | trader  | id        | type | volume | price | resulting trades | type  | tif |
      | trader2 | ETH/DEC19 | buy  |    100 |   180 |                0 | LIMIT | TIF_GTC |
      | trader3 | ETH/DEC19 | sell |    100 |   180 |                1 | LIMIT | TIF_GTC |
