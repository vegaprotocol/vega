Feature: Test vAMM cancellation without position works as expected.
  ...
  @VAMM
  Scenario: 0090-VAMM-022: If a vAMM is cancelled and set in Reduce-Only mode when it currently has no position then all funds are released after the next mark to market.
