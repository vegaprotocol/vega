...
func NewTradableInstrument(log *logging.Logger, pti *types.TradableInstrument, oe products.OracleEngine) (*TradableInstrument, error) {
	instrument, err := NewInstrument(log, pti.Instrument, oe)
	if err != nil {
		return nil, err
	}
...
func NewInstrument(log *logging.Logger, pi *types.Instrument, oe products.OracleEngine) (*Instrument, error) {
	product, err := products.New(log, pi.Product, oe)
	if err != nil {
