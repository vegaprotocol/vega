Feature: Spot market
  ...
  Scenario: Persistent order results in an auction (one trigger breached), no orders placed during auction,
            auction terminates with a trade from order that originally triggered the auction. (0032-PRIM-025)
  ...
  # Wait for the auction to finish
   ...
   And the orders should have the following states:
    | party  | market id | reference | side | volume | remaining | price | status        |
    | party1 | BTC/ETH   | pr-buy    | buy  | 1      | 0         | 1050  | STATUS_FILLED |
    | party5 | BTC/ETH   | pr-sell   | sell | 1      | 0         | 1050  | STATUS_FILLED |

    # Check that the market price bounds are set to higher levels because the mark price has gone up
    And the market data for the market "BTC/ETH" should be:
      | mark price | trading mode            | horizon | min bound | max bound | target stake | supplied stake | open interest |
      | 1050       | TRADING_MODE_CONTINUOUS | 3600    | 1007      | 1095      | 0            | 0              | 0             |
