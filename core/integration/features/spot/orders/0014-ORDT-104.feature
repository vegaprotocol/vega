Feature: Spot market
  ...
  Scenario: In Spot market, cancelling an iceberg order will cancel the order, remove it from the order book,
            release holding asset and update order book to reflect the change (0014-ORDT-104)

    # Place an iceberg order and check the holding account
    When the parties place the following iceberg orders:
      ...
      | party1 | BTC/ETH   | buy  | 10     | 1000  | 0                | TYPE_LIMIT | TIF_GTC | 4         | 1                    | post | iceberg1  |

    And "party1" should have holding account balance of "100" for asset "ETH"
    And "party1" should have general account balance of "9890" for asset "ETH"

    Then the order book should have the following volumes for market "BTC/ETH":
      | side | price | volume |
      | buy  | 1000  | 4      |

    # Cancel the iceberg order and make sure the assets in the holding account are returned
    When the parties cancel the following orders:
      | party  | reference |
      | party1 | iceberg1  |

    And "party1" should have holding account balance of "0" for asset "ETH"
    And "party1" should have general account balance of "9990" for asset "ETH"
