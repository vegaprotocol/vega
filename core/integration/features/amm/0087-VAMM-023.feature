Feature: Test cancelled vAMM becomes active on amend.
  ...
  @VAMM
  Scenario: 0087-VAMM-023: If a vAMM is cancelled and set into Reduce-Only mode, then an amend is sent by the user who created it, the vAMM is amended according to those instructions and is moved out of Reduce-Only mode back into normal operation.
    ...
    And the market data for the market "ETH/MAR22" should be:
      | mark price | trading mode            | horizon | min bound | max bound | target stake | supplied stake | open interest | ref price | mid price | static mid price |
      | 105        | TRADING_MODE_CONTINUOUS | 3600    | 99        | 111       | 461          | 1000           | 11            | 104       | 100       | 100              |
