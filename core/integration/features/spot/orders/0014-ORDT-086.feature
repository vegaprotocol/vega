Feature: Spot market
  ...
  Scenario: No party can submit a network order type (0014-ORDT-086)

    Given time is updated to "2024-01-01T00:01:00Z"

    # Place a GTT order with an expiry time in the past to check it fails.
    And the parties place the following orders:
      | party  | market id | side | volume | price | resulting trades | type         | tif     | reference | error              |
      | party1 | BTC/ETH   | buy  | 1      | 1000  | 0                | TYPE_NETWORK | TIF_GTC | buy1      | invalid order type |
      | party5 | BTC/ETH   | sell | 1      | 1000  | 0                | TYPE_NETWORK | TIF_GTC | sell1     | invalid order type |
