Feature: Spot market
  ...
  Scenario: Amending an iceberg order to increase size will increase the total and remaining quantities of the order and time priority of the order is not lost (0014-ORDT-101)
    ...
    When the parties place the following iceberg orders:
      ...
      | party1 | BTC/ETH   | buy  | 2      | 1000  | 0                | TYPE_LIMIT | TIF_GTC | 2         | 2                    | post | iceberg1  |

    And the parties place the following orders:
      | party  | market id | side | volume | price | resulting trades | type       | tif     | reference |
      | party2 | BTC/ETH   | buy  | 1      | 1000  | 0                | TYPE_LIMIT | TIF_GTC | buy2      |

    # Amend the iceberg to have a bigger size
    When the parties amend the following orders:
      | party  | reference  | price | size delta | tif     | 
      | party1 | iceberg1   | 1000  | 5          | TIF_GTC | 
    ...
    Then the orders should have the following states:
      ...
      | party1 | BTC/ETH   | buy  | 1000  | 2         | 7      | iceberg1  | STATUS_ACTIVE  | 
