...
func testReplayWithSnapshotRestore(t *testing.T) {
	...
	// pretend the protocol restores to block height 15
	...
	// we'll replay 5 blocks and start the 6th
	testChain.app.EXPECT().BeginBlock(gomock.Any()).Times(6)
	...
	// the next begin block should be at block height 17 (restored to 10, replayed 5, processed another 1)
	...
	require.NoError(t, err)
	require.Equal(t, req.Header.Height, int64(17))
	require.Equal(t, req.Header.Time.UnixNano(), genesis.Add(16*time.Second).UnixNano())
	require.Equal(t, req.Header.ChainID, chainID)
}
...
func testReplayFromGenesis(t *testing.T) {
	...
	// we'll replay 5 blocks and start the 16th
	newChain.app.EXPECT().InitChain(gomock.Any()).Times(1)
	newChain.app.EXPECT().BeginBlock(gomock.Any()).Times(16)
...
func getTestNullChain(t *testing.T, txnPerBlock uint64, d time.Duration) *testNullBlockChain {
	...
	nc.app.EXPECT().BeginBlock(gomock.Any()).Times(1)
...
func generateChain(t *testing.T, nc *testNullBlockChain, height int) {
	...
	ctx := context.Background()
	nc.app.EXPECT().InitChain(gomock.Any()).Times(1)
	nc.app.EXPECT().BeginBlock(gomock.Any()).Times(height + 1) // +1 because we start the next block
