...
func TestCalculateRewardsForValidators(t *testing.T) {
	tm := time.Now()
	// no account balance
	require.Nil(t, calculateRewardsForValidators("1", "zohar", "123", num.UintZero(), tm, []*types.PartyContibutionScore{{Party: "z1", Score: num.DecimalFromFloat(0.1)}}, 0))
	// no contributions
	require.Nil(t, calculateRewardsForValidators("1", "zohar", "123", num.NewUint(100), tm, []*types.PartyContibutionScore{}, 0))
	// one contributed
	po := calculateRewardsForValidators("1", "zohar-asset", "123", num.NewUint(100), tm, []*types.PartyContibutionScore{{Party: "z1", Score: num.DecimalFromFloat(0.1)}}, 1)
	require.NotNil(t, po)
	...
	po = calculateRewardsForValidators("1", "zohar-asset", "123", num.NewUint(1000), tm, []*types.PartyContibutionScore{{Party: "z1", Score: num.DecimalFromFloat(0.1)}, {Party: "z2", Score: num.DecimalFromFloat(0.5)}, {Party: "z3", Score: num.DecimalFromFloat(0.2)}}, 1)
	require.NotNil(t, po)
