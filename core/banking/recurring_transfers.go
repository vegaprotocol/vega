	...
	for _, v := range e.recurringTransfers {
		...
		if v.DispatchStrategy == nil {
			resps, err = e.processTransfer(
				ctx, v.From, v.To, v.Asset, "", v.FromAccountType, v.ToAccountType, amount, v.Reference, v.ID, nil, // last is eventual oneoff, which this is not
			)
		} else {
			...
			if _, err = e.ensureFeeForTransferFunds(amount, v.From, v.Asset, v.FromAccountType); err == nil {
				...
				if v.DispatchStrategy.Metric == vegapb.DispatchMetric_DISPATCH_METRIC_MARKET_VALUE {
					...
					for _, fms := range marketProposersScore {
						...
						r, err = e.processTransfer(
							ctx, v.From, v.To, v.Asset, fms.Market, v.FromAccountType, v.ToAccountType, amt, v.Reference, v.ID, nil, // last is eventual oneoff, which this is not
						)
						if err != nil {
				...
				if e.dispatchRequired(v.DispatchStrategy) {
					...
					r, err = e.processTransfer(
						ctx, v.From, v.To, v.Asset, hash, v.FromAccountType, v.ToAccountType, amount, v.Reference, v.ID, nil, // last is eventual oneoff, which this is not
					)
					if err != nil {
