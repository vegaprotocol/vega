Feature: Price monitoring test for issue 2681
  ...
  Scenario: Upper bound breached
    ...
    # T0 + 3min 
    # So that the reference price from which the bounds are computed is 649
    Then the time is updated to "2020-10-16T00:03:00Z" 

    Then traders place following orders:
      | trader  | id        | type | volume |   price    | resulting trades | type       | tif     |
      | trader1 | ETH/DEC20 | sell |      1 |   6770000  |                0 | TYPE_LIMIT | TIF_GTC |
      | trader2 | ETH/DEC20 | buy  |      1 |   6770000  |                1 | TYPE_LIMIT | TIF_FOK |

    And the mark price for the market "ETH/DEC20" is "6770000"

    And the market state for the market "ETH/DEC20" is "MARKET_STATE_CONTINUOUS"

    Then traders place following orders:
      | trader  | id        | type | volume |   price    | resulting trades | type       | tif     |
      | trader1 | ETH/DEC20 | sell |      1 |   6790000  |                0 | TYPE_LIMIT | TIF_GTC |
      | trader2 | ETH/DEC20 | buy  |      1 |   6790000  |                0 | TYPE_LIMIT | TIF_FOK |
