...
func (tw *TimeWeightedNotionalPosition) Upsert(ctx context.Context, twNotionalPos entities.TimeWeightedNotionalPosition) error {
	_, err := tw.Connection.Exec(ctx, `
		INSERT INTO time_weighted_notional_positions (asset_id, party_id, epoch_seq, time_weighted_notional_position, vega_time)
		VALUES ($1, $2, $3, $4, $5)
		ON CONFLICT (asset_id, party_id, epoch_seq, vega_time)
		DO UPDATE
...
func (tw *TimeWeightedNotionalPosition) Get(ctx context.Context, assetID entities.AssetID, partyID entities.PartyID, epochSeq *uint64) (entities.TimeWeightedNotionalPosition, error) {
	...
	if epochSeq == nil {
		err := pgxscan.Get(ctx, tw.Connection, &twNotionalPos,
			`SELECT * FROM time_weighted_notional_positions WHERE asset_id = $1 AND party_id = $2 ORDER BY epoch_seq DESC, vega_time DESC LIMIT 1`,
			assetID, partyID)
		if err != nil {
	...
	err := pgxscan.Get(ctx, tw.Connection, &twNotionalPos,
		`SELECT * FROM time_weighted_notional_positions WHERE asset_id = $1 AND party_id = $2 AND epoch_seq = $3 ORDER BY vega_time DESC LIMIT 1`,
