...
func TestMarkets(t *testing.T) {
	queries := map[string]string{
		...
		// TODO: Something strange going on with LPs - e.g. in the 'buys' field it refers to orders
		// that seemingly don't exist in the orders table; maybe a borked event file?
	...
	for name, query := range queries {
		t.Run(name, func(t *testing.T) {
			assertGraphQLQueriesReturnSame[struct{ Markets []Market }](t, query)
		})
	}
}
