...
func testGetTraderAccountsSuccess(t *testing.T) {
	...
	svc.storage.EXPECT().GetPartyAccounts(owner, "", "", types.AccountType_NO_ACC).Times(1).Return(all, nil)
	accs, err := svc.GetPartyAccounts(owner, "", "", types.AccountType_NO_ACC)
	...
	svc.storage.EXPECT().GetPartyAccounts(owner, market1, "", types.AccountType_NO_ACC).Times(1).Return(firstMarket[:2], nil)
	accs, err = svc.GetPartyAccounts(owner, market1, "", types.AccountType_NO_ACC)
	assert.NoError(t, err)
...
func testGetTraderAccountsErr(t *testing.T) {
	...
	defer svc.ctrl.Finish()
	owner := "test"
	svc.storage.EXPECT().GetPartyAccounts(owner, "", "", types.AccountType_NO_ACC).Times(1).Return(nil, storage.ErrOwnerNotFound)
	accs, err := svc.GetPartyAccounts(owner, "", "", types.AccountType_NO_ACC)
	assert.Error(t, err)
	assert.Nil(t, accs)
	...
	market := "BTC/DEC19"
	svc.storage.EXPECT().GetPartyAccounts(owner, market, "", types.AccountType_NO_ACC).Times(1).Return(nil, storage.ErrOwnerNotFound)
	accs, err = svc.GetPartyAccounts(owner, market, "", types.AccountType_NO_ACC)
	assert.Nil(t, accs)
	assert.Error(t, err)

	// check we're returning the correct error
	assert.Equal(t, storage.ErrOwnerNotFound, err)
	svc.storage.EXPECT().GetPartyAccounts(owner, market, "", types.AccountType_NO_ACC).Times(1).Return(nil, storage.ErrMarketNotFound)
	accs, err = svc.GetPartyAccounts(owner, market, "", types.AccountType_NO_ACC)
	assert.Nil(t, accs)
