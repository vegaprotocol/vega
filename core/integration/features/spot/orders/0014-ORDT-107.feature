Feature: Spot market
  ...
  Scenario: A passive iceberg order (the only order at a particular price level) when crossed with another order that comes in which
            consumes the full volume of the iceberg order is fully filled. Status of iceberg order is filled and the remaining = 0.
            Atomic trades are produced (0014-ORDT-107)
    ...
    # Place order at the same price level to fill it
    And the parties place the following orders:
      | party  | market id | side | volume | price | resulting trades | type       | tif     | reference |
      | party5 | BTC/ETH   | sell | 20     | 1000  | 1                | TYPE_LIMIT | TIF_GTC | sell1     |

    Then the orders should have the following states:
      | party  | market id | side | price | remaining | volume | reference | status         |
      | party1 | BTC/ETH   | buy  | 1000  | 0         | 20     | iceberg1  | STATUS_FILLED  | 
      | party5 | BTC/ETH   | sell | 1000  | 0         | 20     | sell1     | STATUS_FILLED  |             
