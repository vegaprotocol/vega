...
func testSettleExpiredSuccess(t *testing.T) {
	...
	engine.Update(positions)
	// now settle:
	got, _, err := engine.Settle(time.Now(), 0)
	assert.NoError(t, err)
	assert.Equal(t, len(expect), len(got))
	for i, p := range got {
		e := expect[i]
...
func testSettleExpiryFail(t *testing.T) {
	...
	engine.Update(positions)
	empty, _, err := engine.Settle(time.Now(), 0)
...
func TestConcurrent(t *testing.T) {
	...
	engine := getTestEngine(t)
	defer engine.Finish()
	engine.prod.EXPECT().Settle(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().DoAndReturn(func(price *num.Uint, assetDecimals uint32, size num.Decimal) (*types.FinancialAmount, bool, num.Decimal, error) {
